...
export const getSubgroup = (supplied:number, subgroups:Array<Subgroup>):string => {
  const correspondingSubgroup = subgroups?.find((subgroup: Subgroup) => supplied >= subgroup.min && supplied <= subgroup.max)
